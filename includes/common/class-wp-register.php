 ...
 */
class WP_Register extends Captcha {
	...
	public function __construct() {
		...
		add_filter( 'registration_errors', array( $this, 'validate_recaptcha' ) );
	}

	public static function is_allowed( $args = null ) {
		$enable = false;

		if ( self::are_keys_valid() && UM()->options()->get( 'g_recaptcha_wp_register_form' ) ) {
			$enable = true;
		}
	...
	public function add_recaptcha() {
		...
		echo wp_kses( self::render_wp_captcha( array( 'mode' => 'register' ) ), UM()->get_allowed_html( 'templates' ) );
	}
		...
		switch ( $version ) {
			case 'v3':
				...
				if ( ! empty( $result ) ) {
					...
					if ( isset( $result->action ) && 'register' !== $result->action ) {
